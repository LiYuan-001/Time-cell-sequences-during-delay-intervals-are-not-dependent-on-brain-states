...
% arm 16,34 takes 4*harm space
yA16 = [min(maze.locs.A16(:,2)):dd:max(maze.locs.A16(:,2))]';
xMax = max(maze.locs.A16(:,1));
xMin = min(maze.locs.A16(:,1));
xA16 = (xMax -(xMax-xMin)/6)*ones(length(yA16),1);
...
yMax = max(maze.locs.A12(:,2));
yMin = min(maze.locs.A12(:,2));
yA12 = (yMin +((yMax-yMin)/6))*ones(length(xA12),1);
...
yMax = max(maze.locs.A56(:,2));
yMin = min(maze.locs.A56(:,2));
yA56 = (yMax -((yMax-yMin)/6))*ones(length(xA56),1);
