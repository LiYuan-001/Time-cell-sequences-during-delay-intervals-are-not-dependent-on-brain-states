...
function Fig8TreadmillDelayField_TrialbyTrial_2Session(inFile,AnalyzeSes)
close all
...
for i = AnalyzeSes(1:end)
    ...
    % initiate the data
    DelayField_TrialbyTrial_2Session.rat = sessInfo(i).animal;
    DelayField_TrialbyTrial_2Session.day = sessInfo(i).day;
    DelayField_TrialbyTrial_2Session.timeBin = DelayFire.timeBin;
    DelayField_TrialbyTrial_2Session.gaussSigma = DelayFire.gaussSigma;
    DelayField_TrialbyTrial_2Session.peakThres = p.peak;

    Fig8DelayTimeMap_2Session.rat = sessInfo(i).animal;
    Fig8DelayTimeMap_2Session.day = sessInfo(i).day;
    Fig8DelayTimeMap_2Session.timeBin = DelayFire.timeBin;
    Fig8DelayTimeMap_2Session.gaussSigma = DelayFire.gaussSigma;
     
    TList = DelayFire.tList;
    clusterNum = length(DelayFire.tList);
    DelayField_TrialbyTrial_2Session.tList = DelayFire.tList;
    Fig8DelayTimeMap_2Session.tList = DelayFire.tList;
    ...
    for j = 1:length(sessDirs)
        ...
        trialNumTemp = size(DelayFire.(sessDirs{j}).delayTstart1,2);
        ...
        binCount = size(DelayFire.(sessDirs{j}).spikeRate1_Smooth{1},2);
        prebinCount = size(DelayFire.(sessDirs{j}).pre_spikeRate1_Smooth{1},2);
        postbinCount = size(DelayFire.(sessDirs{j}).post_spikeRate1_Smooth{1},2);
        % -----------------------------------------------------------------
        for k = 1:clusterNum
            ...
            premap_1.(sessName)(trialInd,1:prebinCount,k) = DelayFire.(sessDirs{j}).pre_spikeRate1_Smooth{k};
            premap_2.(sessName)(trialInd,1:prebinCount,k) = DelayFire.(sessDirs{j}).pre_spikeRate2_Smooth{k};
            
            postmap_1.(sessName)(trialInd,1:postbinCount,k) = DelayFire.(sessDirs{j}).post_spikeRate1_Smooth{k};
        end
    end
    ...
    for j = 1:length(sessName2)
        ...
        shuffleSig_2 = DelayFireStability_TrialbyTrial_2Session.(sessName2{j}).delayCorr_Def2_Trial_Shuffle_Label95;
        ...
        for k = 1:clusterNum
            ...
            % get post map
            post_spikeRate1_2session = postmap_1.(sessName2{j})(:,:,k);
            post_mapMean_Def1_2Session = nanmean(post_spikeRate1_2session,1);
            % put map into the .mat file
            Fig8DelayTimeMap_2Session.(sessName2{j}).spikeRate1_Smooth{k} = spikeRate1_2session;
            Fig8DelayTimeMap_2Session.(sessName2{j}).spikeRate1_Combined_Smooth{k} = mapMean_Def1_2Session;
            
            Fig8DelayTimeMap_2Session.(sessName2{j}).spikeRate2_Smooth{k} = spikeRate2_2session;
            Fig8DelayTimeMap_2Session.(sessName2{j}).spikeRate2_Combined_Smooth{k} = mapMean_Def2_2Session;
            
            Fig8DelayTimeMap_2Session.(sessName2{j}).pre_spikeRate1_Smooth{k} = pre_spikeRate1_2session;
            Fig8DelayTimeMap_2Session.(sessName2{j}).pre_spikeRate1_Combined_Smooth{k} = pre_mapMean_Def1_2Session;
            
            Fig8DelayTimeMap_2Session.(sessName2{j}).pre_spikeRate2_Smooth{k} = pre_spikeRate2_2session;
            Fig8DelayTimeMap_2Session.(sessName2{j}).pre_spikeRate2_Combined_Smooth{k} = pre_mapMean_Def2_2Session;
            
            Fig8DelayTimeMap_2Session.(sessName2{j}).post_spikeRate1_2session{k} = post_spikeRate1_2session;
            Fig8DelayTimeMap_2Session.(sessName2{j}).post_mapMean_Def1_2Session{k} = post_mapMean_Def1_2Session;
...
%             if shuffleSig_2(k) == 1 && (peakRate_2 >= (avgRate2 + p.sdThreshold*std_Def2_2Session))
            ...
            if j == 1
                TITLE1 = sprintf('%s%d%s%d%s%s%s%s','DelayZone1-',sessInfo(i).animal,' Day-',sessInfo(i).day,'-',TList{k},'-',sessName2{j});
            else
                TITLE1 = sessName2{j};
            end
        ...
        DelayField_TrialbyTrial_2Session.(sessName2{j}).timeField_2 = timeField_2;        
        DelayField_TrialbyTrial_2Session.(sessName2{j}).fieldLabel_2 = fieldLabel_2;
    end
    ...
    close all
    if p.writeToFile == 1
        save(fullfile(savedir2,'DelayField_TrialbyTrial_2Session.mat'), 'DelayField_TrialbyTrial_2Session');
        save(fullfile(savedir2,'Fig8DelayTimeMap_2Session.mat'), 'Fig8DelayTimeMap_2Session');
    end
    clear DelayField_TrialbyTrial_Shuffle Fig8DelayTimeMap_2Session map_1 map_2
