function Fig8TreadmillMapPerTrial_PCAICA_Delay(inFile,AnalyzeSes)
% 
...
for i = AnalyzeSes(1:end)
    ...
    if p.savePlot
        ...
        savedir = sprintf('%s%s%d%s%d%s',cd,'\Figures\',sessInfo(i).animal,'-day',sessInfo(i).day,'\Assembly_spatial_map_Trial_Delay');
...
%             pinfo = parsingInfo.(SessDirs{j});
            ...
            plin = pathDataLin.(sessDirs{j});
                ...
                Delay_assemblyRatesByECLR.maps = M;
                Delay_assemblyRatesByECLR.xRange = xrange;
                Delay_assemblyRatesByECLR.ECLR = mux(strcmpi('R', tinfo.direction), tinfo.success);
                
                Delay_assemblyRatesByECLR.spikemap = arrayfun(@(str) str.spikemap, preRM, 'un', 0);
                Delay_assemblyRatesByECLR.occupmap = arrayfun(@(str) str.occupmap, preRM, 'un', 0);
                Delay_assemblyRatesByECLR.valid = ~tinfo.degen;
                
                % calculate average linear map of left correct turn and
                % right correct turn
                
                if p.writeToFile
                    save(fullfile(sessInfo(i).mainDir,sessDirs{j}, 'Delay_AssemblyRatesByECLR.mat'), 'Delay_assemblyRatesByECLR');
                end
...
%                 [avgRates, zonespikes] = plotTrialPerCell.computeAvgRates2(avgRates, tSp, tinfo, sessInfo, i, TList, SessDirs, group, eegx, opt, pdata, tridx, trtype);
                ...
                clear Delay_assemblyRatesByECLR
        end
