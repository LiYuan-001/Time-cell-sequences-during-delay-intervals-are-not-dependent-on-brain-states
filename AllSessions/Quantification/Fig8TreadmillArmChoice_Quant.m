...
function Fig8TreadmillArmChoice_Quant(inFile,AnalyzeSes)
close all
figureCount = 0;
ArmRateChoice.rate = [];
p.avgRateThres = 0.5;

sessDirs = {'on10','on30','off10','off30'};
...
%     SigArmChoice_BaseLine_Dist.(sessDirs{j}).choice = h.Values; 
    ...
    hold on
    bar(2,100*sum(return_onIdx & SigArmChoice.off10.returnRateLRDiffAll)/sum(return_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(3,100*sum(return_onIdx & SigArmChoice.on30.returnRateLRDiffAll)/sum(return_onIdx),'FaceColor',[1,0,0.2]);
    bar(4,100*sum(return_onIdx & SigArmChoice.off30.returnRateLRDiffAll)/sum(return_onIdx),'FaceColor',[0.6,0.6,0.6]);
    
    bar(6,100*sum(delay_onIdx & SigArmChoice.on10.delayRateLRDiffAll)/sum(delay_onIdx),'FaceColor',[1,0,0.2]);
    hold on
    bar(7,100*sum(delay_onIdx & SigArmChoice.off10.delayRateLRDiffAll)/sum(delay_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(8,100*sum(delay_onIdx & SigArmChoice.on30.delayRateLRDiffAll)/sum(delay_onIdx),'FaceColor',[1,0,0.2]);
    bar(9,100*sum(delay_onIdx & SigArmChoice.off30.delayRateLRDiffAll)/sum(delay_onIdx),'FaceColor',[0.6,0.6,0.6]);
    
    bar(11,100*sum(stem_onIdx & SigArmChoice.on10.stemRateLRDiffAll)/sum(stem_onIdx),'FaceColor',[1,0,0.2]);
    hold on
    bar(12,100*sum(stem_onIdx & SigArmChoice.off10.stemRateLRDiffAll)/sum(stem_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(13,100*sum(stem_onIdx & SigArmChoice.on30.stemRateLRDiffAll)/sum(stem_onIdx),'FaceColor',[1,0,0.2]);
    bar(14,100*sum(stem_onIdx & SigArmChoice.off30.stemRateLRDiffAll)/sum(stem_onIdx),'FaceColor',[0.6,0.6,0.6]);
    
    bar(16,100*sum(choice_onIdx & SigArmChoice.on10.ChoRateLRDiffAll)/sum(choice_onIdx),'FaceColor',[1,0,0.2]);
    hold on
    bar(17,100*sum(choice_onIdx & SigArmChoice.off10.ChoRateLRDiffAll)/sum(choice_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(18,100*sum(choice_onIdx & SigArmChoice.on30.ChoRateLRDiffAll)/sum(choice_onIdx),'FaceColor',[1,0,0.2]);
    bar(19,100*sum(choice_onIdx & SigArmChoice.off30.ChoRateLRDiffAll)/sum(choice_onIdx),'FaceColor',[0.6,0.6,0.6]);
