...
function Fig8TreadmillArmChoice_Block_Quant(inFile,AnalyzeSes)
close all
figureCount = 0;
ArmRateChoice.rate = [];
p.avgRateThres = 0.5;

sessDirs = {'on10_1','off10_1','on30_1','off30_1','on10_2','off10_2','on30_2','off30_2'};
...
for i = AnalyzeSes(1:end)
    ...
    load(SpikeShapeFile);
    Fig8ArmChoice = fullfile(sessInfo(i).mainDir,'Cell Property','Fig8TreadmillArmChoice.mat');
    load(Fig8ArmChoice);  
...
%     load(Fig8ArmChoice_BaseLine);  
    ...
    for j = 1:length(sessDirs)
        Fig8TreadmillArmChoice.on10_1.delayRateLRDiffAll
        % rate comparison
        ArmRateChoice.(sessDirs{j}).delayRateLRDiffAll = [ArmRateChoice.(sessDirs{j}).delayRateLRDiffAll,...
                Fig8TreadmillArmChoice.(sessDirs{j}).delayRateLRDiffAll(idx)];            
        ArmRateChoice.(sessDirs{j}).stemRateLRDiffAll = [ArmRateChoice.(sessDirs{j}).stemRateLRDiffAll,...
                Fig8TreadmillArmChoice.(sessDirs{j}).stemRateLRDiffAll(idx)];
        ArmRateChoice.(sessDirs{j}).returnRateLRDiffAll = [ArmRateChoice.(sessDirs{j}).returnRateLRDiffAll,...
                Fig8TreadmillArmChoice.(sessDirs{j}).returnRateLRDiffAll(idx)];
        ArmRateChoice.(sessDirs{j}).ChoRateLRDiffAll = [ArmRateChoice.(sessDirs{j}).ChoRateLRDiffAll,...
                Fig8TreadmillArmChoice.(sessDirs{j}).ChoRateLRDiffAll(idx)];
            
%         ArmRateChoice_BaseLine.(sessDirs{j}).delayRateLRDiffAll = [ArmRateChoice_BaseLine.(sessDirs{j}).delayRateLRDiffAll,...
%                 Fig8TreadmillArmChoice_Baseline.(sessDirs{j}).delayRateLRDiffAll(:,idx)];            
%         ArmRateChoice_BaseLine.(sessDirs{j}).stemRateLRDiffAll = [ArmRateChoice_BaseLine.(sessDirs{j}).stemRateLRDiffAll,...
%                 Fig8TreadmillArmChoice_Baseline.(sessDirs{j}).stemRateLRDiffAll(:,idx)];
%         ArmRateChoice_BaseLine.(sessDirs{j}).returnRateLRDiffAll = [ArmRateChoice_BaseLine.(sessDirs{j}).returnRateLRDiffAll,...
%                 Fig8TreadmillArmChoice_Baseline.(sessDirs{j}).returnRateLRDiffAll(:,idx)];
%         ArmRateChoice_BaseLine.(sessDirs{j}).ChoRateLRDiffAll = [ArmRateChoice_BaseLine.(sessDirs{j}).ChoRateLRDiffAll,...
...
%     h = histogram(SigArmChoice_BaseLine.(sessDirs{j}).returnRateLRDiffAll,0:1:20);
%     SigArmChoice_BaseLine_Dist.(sessDirs{j}).return = h.Values;
%     h = histogram(SigArmChoice_BaseLine.(sessDirs{j}).delayRateLRDiffAll,0:1:20);
%     SigArmChoice_BaseLine_Dist.(sessDirs{j}).delay = h.Values; 
%     h = histogram(SigArmChoice_BaseLine.(sessDirs{j}).stemRateLRDiffAll,0:1:20);
%     SigArmChoice_BaseLine_Dist.(sessDirs{j}).stem = h.Values;
%     h = histogram(SigArmChoice_BaseLine.(sessDirs{j}).ChoRateLRDiffAll,0:1:20);
%     SigArmChoice_BaseLine_Dist.(sessDirs{j}).choice = h.Values; 
end



    figure
    bar(1,100*sum(return_onIdx & SigArmChoice.on10_1.returnRateLRDiffAll)/sum(return_onIdx),'FaceColor',[1,0,0.2]);
    hold on
    bar(2,100*sum(return_onIdx & SigArmChoice.on10_2.returnRateLRDiffAll)/sum(return_onIdx),'FaceColor',[1,0,0.2]);
    bar(3,100*sum(return_onIdx & SigArmChoice.off10_1.returnRateLRDiffAll)/sum(return_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(4,100*sum(return_onIdx & SigArmChoice.off10_2.returnRateLRDiffAll)/sum(return_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(5,100*sum(return_onIdx & SigArmChoice.on30_1.returnRateLRDiffAll)/sum(return_onIdx),'FaceColor',[1,0,0.2]);
    bar(6,100*sum(return_onIdx & SigArmChoice.on30_2.returnRateLRDiffAll)/sum(return_onIdx),'FaceColor',[1,0,0.2]);
    bar(7,100*sum(return_onIdx & SigArmChoice.off30_1.returnRateLRDiffAll)/sum(return_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(8,100*sum(return_onIdx & SigArmChoice.off30_2.returnRateLRDiffAll)/sum(return_onIdx),'FaceColor',[0.6,0.6,0.6]);
    
    bar(10,100*sum(delay_onIdx & SigArmChoice.on10_1.delayRateLRDiffAll)/sum(delay_onIdx),'FaceColor',[1,0,0.2]);
    bar(11,100*sum(delay_onIdx & SigArmChoice.on10_2.delayRateLRDiffAll)/sum(delay_onIdx),'FaceColor',[1,0,0.2]);
    hold on
    bar(12,100*sum(delay_onIdx & SigArmChoice.off10_1.delayRateLRDiffAll)/sum(delay_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(13,100*sum(delay_onIdx & SigArmChoice.off10_2.delayRateLRDiffAll)/sum(delay_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(14,100*sum(delay_onIdx & SigArmChoice.on30_1.delayRateLRDiffAll)/sum(delay_onIdx),'FaceColor',[1,0,0.2]);
    bar(15,100*sum(delay_onIdx & SigArmChoice.on30_2.delayRateLRDiffAll)/sum(delay_onIdx),'FaceColor',[1,0,0.2]);
    bar(16,100*sum(delay_onIdx & SigArmChoice.off30_1.delayRateLRDiffAll)/sum(delay_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(17,100*sum(delay_onIdx & SigArmChoice.off30_2.delayRateLRDiffAll)/sum(delay_onIdx),'FaceColor',[0.6,0.6,0.6]);
    
    bar(19,100*sum(stem_onIdx & SigArmChoice.on10_1.stemRateLRDiffAll)/sum(stem_onIdx),'FaceColor',[1,0,0.2]);
    hold on
    bar(20,100*sum(stem_onIdx & SigArmChoice.on10_2.stemRateLRDiffAll)/sum(stem_onIdx),'FaceColor',[1,0,0.2]);
    bar(21,100*sum(stem_onIdx & SigArmChoice.off10_1.stemRateLRDiffAll)/sum(stem_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(22,100*sum(stem_onIdx & SigArmChoice.off10_2.stemRateLRDiffAll)/sum(stem_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(23,100*sum(stem_onIdx & SigArmChoice.on30_1.stemRateLRDiffAll)/sum(stem_onIdx),'FaceColor',[1,0,0.2]);
    bar(24,100*sum(stem_onIdx & SigArmChoice.on30_2.stemRateLRDiffAll)/sum(stem_onIdx),'FaceColor',[1,0,0.2]);
    bar(25,100*sum(stem_onIdx & SigArmChoice.off30_1.stemRateLRDiffAll)/sum(stem_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(26,100*sum(stem_onIdx & SigArmChoice.off30_2.stemRateLRDiffAll)/sum(stem_onIdx),'FaceColor',[0.6,0.6,0.6]);
    
    bar(27,100*sum(choice_onIdx & SigArmChoice.on10_1.ChoRateLRDiffAll)/sum(choice_onIdx),'FaceColor',[1,0,0.2]);
    hold on
    bar(28,100*sum(choice_onIdx & SigArmChoice.on10_2.ChoRateLRDiffAll)/sum(choice_onIdx),'FaceColor',[1,0,0.2]);
    bar(29,100*sum(choice_onIdx & SigArmChoice.off10_1.ChoRateLRDiffAll)/sum(choice_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(30,100*sum(choice_onIdx & SigArmChoice.off10_2.ChoRateLRDiffAll)/sum(choice_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(31,100*sum(choice_onIdx & SigArmChoice.on30_1.ChoRateLRDiffAll)/sum(choice_onIdx),'FaceColor',[1,0,0.2]);
    bar(32,100*sum(choice_onIdx & SigArmChoice.on30_2.ChoRateLRDiffAll)/sum(choice_onIdx),'FaceColor',[1,0,0.2]);
    bar(33,100*sum(choice_onIdx & SigArmChoice.off30_1.ChoRateLRDiffAll)/sum(choice_onIdx),'FaceColor',[0.6,0.6,0.6]);
    bar(34,100*sum(choice_onIdx & SigArmChoice.off30_2.ChoRateLRDiffAll)/sum(choice_onIdx),'FaceColor',[0.6,0.6,0.6]);
    set(gca,'XTick',[1:4,6:9,11:14,16:19],'XTickLabel',{'on10','off10','on30','off30'});
