...
function Fig8TreadmillArmChoice_PCAICA_Quant(inFile,AnalyzeSes)
close all
figureCount = 0;
ArmPCAChoice.rate = [];
...
for i = AnalyzeSes(1:end)
    ...
    Fig8ArmChoice = fullfile(sessInfo(i).mainDir,'Cell Property','Assembly_ArmChoice_SameDelay-25ms.mat');
    load(Fig8ArmChoice);
    
    if isfield(Assembly_ArmChoice_SameDelay,'on10')
        ...
        for j = 1:length(sessDirs)
            ...
            ArmPCAChoice.(sessDirs{j}).delayRateLRDiffAll = [ArmPCAChoice.(sessDirs{j}).delayRateLRDiffAll,...
                Assembly_ArmChoice_SameDelay.(sessDirs{j}).delayRateLRDiffAll];
            
            ArmPCAChoice.(sessDirs{j}).stemRateLRDiffAll = [ArmPCAChoice.(sessDirs{j}).stemRateLRDiffAll,...
                Assembly_ArmChoice_SameDelay.(sessDirs{j}).stemRateLRDiffAll];
            
            ArmPCAChoice.(sessDirs{j}).ChoRateLRDiffAll = [ArmPCAChoice.(sessDirs{j}).ChoRateLRDiffAll,...
                Assembly_ArmChoice_SameDelay.(sessDirs{j}).stemChoRateLRDiffAll];
            
            ArmPCAChoice.(sessDirs{j}).returnRateLRDiffAll = [ArmPCAChoice.(sessDirs{j}).returnRateLRDiffAll,...
                Assembly_ArmChoice_SameDelay.(sessDirs{j}).returnRateLRDiffAll];
            
            % 95% value
            
            ArmPCAChoice.(sessDirs{j}).shf_delayRateLRDiffAll95 = [ArmPCAChoice.(sessDirs{j}).shf_delayRateLRDiffAll95,...
                Assembly_ArmChoice_SameDelay.(sessDirs{j}).shf_delayRateLRDiffAll95];
            
            ArmPCAChoice.(sessDirs{j}).shf_stemRateLRDiffAll95 = [ArmPCAChoice.(sessDirs{j}).shf_stemRateLRDiffAll95,...
                Assembly_ArmChoice_SameDelay.(sessDirs{j}).shf_stemRateLRDiffAll95];
            
            ArmPCAChoice.(sessDirs{j}).shf_ChoRateLRDiffAll95 = [ArmPCAChoice.(sessDirs{j}).shf_ChoRateLRDiffAll95,...
                Assembly_ArmChoice_SameDelay.(sessDirs{j}).shf_ChoRateLRDiffAll95];
            
            ArmPCAChoice.(sessDirs{j}).shf_returnRateLRDiffAll95 = [ArmPCAChoice.(sessDirs{j}).shf_returnRateLRDiffAll95,...
                Assembly_ArmChoice_SameDelay.(sessDirs{j}).shf_returnRateLRDiffAll95];
        end
        
        % delay active assemblies
        delayActive_on10 = Fig8TreadmillArmRate_SameDelay_Assembly.on10.rateDelay>0.1;
        delayActive_off10 = Fig8TreadmillArmRate_SameDelay_Assembly.on30.rateDelay>0.1;
        delayActive_on30 = Fig8TreadmillArmRate_SameDelay_Assembly.off10.rateDelay>0.1;
        delayActive_off30 = Fig8TreadmillArmRate_SameDelay_Assembly.off30.rateDelay>0.1;
        delayActiveTemp = (delayActive_on10 + delayActive_off10 + delayActive_on30 + delayActive_off30)>0;
        
        % return active assemblies
        returnActive_on10 = (Fig8TreadmillArmRate_SameDelay_Assembly.on10.rateReturn + Fig8TreadmillArmRate_SameDelay_Assembly.on10.rateBase)>0.1;
        returnActive_off10 = (Fig8TreadmillArmRate_SameDelay_Assembly.off10.rateReturn + Fig8TreadmillArmRate_SameDelay_Assembly.off10.rateBase)>0.1;
        returnActive_on30 = (Fig8TreadmillArmRate_SameDelay_Assembly.on30.rateReturn + Fig8TreadmillArmRate_SameDelay_Assembly.on30.rateBase)>0.1;
        returnActive_off30 = (Fig8TreadmillArmRate_SameDelay_Assembly.off30.rateReturn + Fig8TreadmillArmRate_SameDelay_Assembly.off30.rateBase)>0.1;
        returnActiveTemp  = (returnActive_on10 + returnActive_off10 + returnActive_on30 + returnActive_off30)>0;
        
        % stem active assemblies
        stemActive_on10 = (Fig8TreadmillArmRate_SameDelay_Assembly.on10.rateStem )>0.1;
        stemActive_off10 = (Fig8TreadmillArmRate_SameDelay_Assembly.off10.rateStem)>0.1;
        stemActive_on30 = (Fig8TreadmillArmRate_SameDelay_Assembly.on30.rateStem)>0.1;
        stemActive_off30 = (Fig8TreadmillArmRate_SameDelay_Assembly.off30.rateStem)>0.1;
        stemActiveTemp  = (stemActive_on10 + stemActive_off10 + stemActive_on30 + stemActive_off30)>0;
        
        % choice active assemblies
        choiceActive_on10 = Fig8TreadmillArmRate_SameDelay_Assembly.on10.rateChoice>0.1;
        choiceActive_off10 = Fig8TreadmillArmRate_SameDelay_Assembly.off10.rateChoice>0.1;
        choiceActive_on30 = Fig8TreadmillArmRate_SameDelay_Assembly.on30.rateChoice>0.1;
        choiceActive_off30 = Fig8TreadmillArmRate_SameDelay_Assembly.off30.rateChoice>0.1;
