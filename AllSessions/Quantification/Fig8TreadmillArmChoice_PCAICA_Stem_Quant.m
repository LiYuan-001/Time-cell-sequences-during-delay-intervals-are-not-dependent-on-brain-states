...
function Fig8TreadmillArmChoice_PCAICA_Stem_Quant(inFile,AnalyzeSes)
close all
figureCount = 0;
ArmPCAChoice.rate = [];
...
for i = AnalyzeSes(1:end)
    ...
    Fig8ArmChoice = fullfile(sessInfo(i).mainDir,'Cell Property','Assembly_ArmChoice_Stem_25ms.mat');
    load(Fig8ArmChoice);
    Delay_OnOff_File = fullfile(sessInfo(i).mainDir,'Cell Property','CellAssembly_Stem-25ms.mat');
    load(Delay_OnOff_File);
    
    if isfield(Assembly_ArmChoice_Stem,'on10')
...
%   
        for j = 1:length(sessDirs)
            for n = 1:length(assemblyCat)
                if isfield(Assembly_ArmChoice_Stem.(sessDirs{j}),assemblyCat{n})
                %             % delay Rate
                ...
                ArmPCAChoice.(sessDirs{j}).(assemblyCat{n}).shf_delayRateLRDiffAll95 = [ArmPCAChoice.(sessDirs{j}).(assemblyCat{n}).shf_delayRateLRDiffAll95,...
                    Assembly_ArmChoice_Stem.(sessDirs{j}).(assemblyCat{n}).shf_delayRateLRDiffAll95];
                ...
                ArmPCAChoice.(sessDirs{j}).(assemblyCat{n}).shf_stemRateLRDiffAll95 = [ArmPCAChoice.(sessDirs{j}).(assemblyCat{n}).shf_stemRateLRDiffAll95,...
                    Assembly_ArmChoice_Stem.(sessDirs{j}).(assemblyCat{n}).shf_stemRateLRDiffAll95];
                ...
                ArmPCAChoice.(sessDirs{j}).(assemblyCat{n}).shf_ChoRateLRDiffAll95 = [ArmPCAChoice.(sessDirs{j}).(assemblyCat{n}).shf_ChoRateLRDiffAll95,...
                    Assembly_ArmChoice_Stem.(sessDirs{j}).(assemblyCat{n}).shf_ChoRateLRDiffAll95];
                ...
                ArmPCAChoice.(sessDirs{j}).(assemblyCat{n}).shf_returnRateLRDiffAll95 = [ArmPCAChoice.(sessDirs{j}).(assemblyCat{n}).shf_returnRateLRDiffAll95,...
                    Assembly_ArmChoice_Stem.(sessDirs{j}).(assemblyCat{n}).shf_returnRateLRDiffAll95];
                end
            end
        end
    end
    
    if CellAssembly_Stem.DelayOn.patNum > 0
        for m = 1:CellAssembly_Stem.DelayOn.patNum
            sim_Temp = [];
            pattern_On = CellAssembly_Stem.DelayOn.AssmblWght(:,m);
            for j = 1:CellAssembly_Stem.DelayOff.patNum
                pattern_Off = CellAssembly_Stem.DelayOff.AssmblWght(:,j);
            ...
            assembly_ID_on = [assembly_ID_on,CellAssembly_Stem.rat*100+CellAssembly_Stem.day*10+m];
        end
    end
    if CellAssembly_Stem.DelayOff.patNum > 0
        for m = 1:CellAssembly_Stem.DelayOff.patNum
            sim_Temp = [];
            pattern_Off = CellAssembly_Stem.DelayOff.AssmblWght(:,m);
            for j = 1:CellAssembly_Stem.DelayOn.patNum
                pattern_On = CellAssembly_Stem.DelayOn.AssmblWght(:,j);
            ...
            assembly_ID_off = [assembly_ID_off,CellAssembly_Stem.rat*100+CellAssembly_Stem.day*10+m];
        end
    end
end
