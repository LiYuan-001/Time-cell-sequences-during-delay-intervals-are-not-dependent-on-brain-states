function Fig8TreadmillArmRate_2(inFile,AnalyzeSes)
...
for i = AnalyzeSes(1:end)
    ...
    if p.savePlot
        ...
        savedir = sprintf('%s%s%d%s%d%s',cd,'\Figures\',sessInfo(i).animal,'-day',sessInfo(i).day,'\ArmRate');
             ...
             % load maps for each cluster
                ...
                if exist(fullfile(sessInfo(i).mainDir,sessDirs{j},'ratesByECLR.mat'), 'file')==2
                    ...
                    for m = 1:sum(map_1D.ratesByECLR.valid)
                        ...
                        % delay
                        timeTemp = Fig8DelayZonePos.delayPos1.endT(m)-Fig8DelayZonePos.delayPos1.startT(m);
                ...
                for k = 1:clusterNum
                    ...
                    for m = 1:sum(map_1D.ratesByECLR.valid)
                        ...
                        % delay
                        spikeInd = (tSp>=Fig8DelayZonePos.delayPos1.startT(m) & tSp<Fig8DelayZonePos.delayPos1.endT(m));
