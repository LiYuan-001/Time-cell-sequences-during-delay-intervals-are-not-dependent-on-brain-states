function Fig8TreadmillArmChoice_Classfier(inFile,AnalyzeSes,shuffleTimes)
...
for i = AnalyzeSes(1:end)
    ...
    ArmChoice_Classfier.session = sessInfo(i).mainDir;
    ArmChoice_Classfier.rat = sessInfo(i).animal;
    ArmChoice_Classfier.day = sessInfo(i).day;
    
    if p.savePlot
        % directory for plot figures
        % generate a folder for each rat eah day under the current folder
        savedir = sprintf('%s%s%d%s%d%s',cd,'\Figures\',sessInfo(i).animal,'-day',sessInfo(i).day,'\ArmChoice_Classifier');
    ...
    ArmChoice_Classfier.TList = TList;
...
%                 Fig8TreadmillCenterRateDiff.(behaveType).LRrateDiff(k) = LRrateDiff;
                ArmChoice_Classfier.(behaveType).returnRateLRDiffAll(k) = returnRateLRDiffAll;
                ArmChoice_Classfier.(behaveType).stemRateLRDiffAll(k) = stemRateLRDiffAll;
                ArmChoice_Classfier.(behaveType).delayRateLRDiffAll(k) = delayRateLRDiffAll;
                ArmChoice_Classfier.(behaveType).ChoRateLRDiffAll(k) = ChoRateLRDiffAll;
...
%                     
                ...
                if length(tempSort) > shuffleTimes/10
                    ...
                    hold off
                    ArmChoice_Classfier.(behaveType).shf_delayRateLRDiffAll(k) = 1;
                else
                    shf_delayRateLRDiffAll95 = NaN;
                    shf_delayRateLRDiffAll99 = NaN;
                    text(0.1,0.5,'Comparison not exist','FontSize',8);
                    ArmChoice_Classfier.(behaveType).shf_delayRateLRDiffAll(k) = 0;
                end
                ArmChoice_Classfier.(behaveType).shf_delayRateLRDiffAll95(k) = shf_delayRateLRDiffAll95;
                ArmChoice_Classfier.(behaveType).shf_delayRateLRDiffAll99(k) = shf_delayRateLRDiffAll99;
                ...
                if length(tempSort) > shuffleTimes/10
                    ...
                    hold off
                    ArmChoice_Classfier.(behaveType).shf_returnRateLRDiffAll(k) = 1;
                else
                    shf_returnRateLRDiffAll95 = NaN;
                    shf_returnRateLRDiffAll99 = NaN;
                    text(0.1,0.5,'Comparison not exist','FontSize',8);
                    ArmChoice_Classfier.(behaveType).shf_returnRateLRDiffAll(k) = 0;
                end
                ArmChoice_Classfier.(behaveType).shf_returnRateLRDiffAll95(k) = shf_returnRateLRDiffAll95;
                ArmChoice_Classfier.(behaveType).shf_returnRateLRDiffAll99(k) = shf_returnRateLRDiffAll99;
                ...
                if length(tempSort) > shuffleTimes/10
                    ...
                    hold off
                    ArmChoice_Classfier.(behaveType).shf_stemRateLRDiffAll(k) = 1;
                else
                    shf_stemRateLRDiffAll95 = NaN;
                    shf_stemRateLRDiffAll99 = NaN;
                    text(0.1,0.5,'Comparison not exist','FontSize',8);
                    ArmChoice_Classfier.(behaveType).shf_stemRateLRDiffAll(k) = 0;
                end
                ArmChoice_Classfier.(behaveType).shf_stemRateLRDiffAll95(k) = shf_stemRateLRDiffAll95;
                ArmChoice_Classfier.(behaveType).shf_stemRateLRDiffAll99(k) = shf_stemRateLRDiffAll99;
                ylabel('Stem (L vs R)')
                xticklabels({''});
                yticklabels({''});
                ...
                if length(tempSort) > shuffleTimes/10
                    ...
                    hold off
                    ArmChoice_Classfier.(behaveType).shf_ChoRateLRDiffAll(k) = 1;
                else
                    shf_ChoRateLRDiffAll95 = NaN;
                    shf_ChoRateLRDiffAll99 = NaN;
                    text(0.1,0.5,'Comparison not exist','FontSize',8);
                    ArmChoice_Classfier.(behaveType).shf_ChoRateLRDiffAll(k) = 0;
                end
                ArmChoice_Classfier.(behaveType).shf_ChoRateLRDiffAll95(k) = shf_ChoRateLRDiffAll95;
                ArmChoice_Classfier.(behaveType).shf_ChoRateLRDiffAll99(k) = shf_ChoRateLRDiffAll99;
