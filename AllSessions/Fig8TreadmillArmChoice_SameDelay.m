function Fig8TreadmillArmChoice_SameDelay(inFile,AnalyzeSes,shuffleTimes)
...
for i = AnalyzeSes(1:end)
    ...
    Fig8TreadmillArmChoice_SameDelay.session = sessInfo(i).mainDir;
    Fig8TreadmillArmChoice_SameDelay.rat = sessInfo(i).animal;
    Fig8TreadmillArmChoice_SameDelay.day = sessInfo(i).day;
    ...
    Fig8TreadmillArmChoice_SameDelay.TList = TList;
            ...
            for j = [sesGroup,sesGroup+4]
                ...
                if exist(fullfile(sessInfo(i).mainDir,sessDirs{j},'ratesByECLR.mat'), 'file')==2
                    ...
                    for m = 1:trialNum
                        ...
                        % return
                        timeTemp = PathZone.posEndT.Base(m)-PathZone.posStartT.Return(m);
            ...
            for k = 1:clusterNum                                   
                ...
                for j = [sesGroup,sesGroup+4]
                    ...
                    for m = 1:sum(map_1D{j}.ratesByECLR.valid)
                        % get spike count
                        % return
                        spikeCount = sum(tSp>=PathZone.posStartT.Return(m) & tSp<PathZone.posEndT.Base(m));
...
%                 Fig8TreadmillCenterRateDiff.(behaveType).LRrateDiff(k) = LRrateDiff;
                Fig8TreadmillArmChoice_SameDelay.(behaveType).returnRateLRDiffAll(k) = returnRateLRDiffAll;
                Fig8TreadmillArmChoice_SameDelay.(behaveType).stemRateLRDiffAll(k) = stemRateLRDiffAll;
                Fig8TreadmillArmChoice_SameDelay.(behaveType).delayRateLRDiffAll(k) = delayRateLRDiffAll;
                Fig8TreadmillArmChoice_SameDelay.(behaveType).ChoRateLRDiffAll(k) = ChoRateLRDiffAll;
...
%                     
                ...
                if length(tempSort) > shuffleTimes/10
                    ...
                    hold off
                    Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_delayRateLRDiffAll(k) = 1;
                else
                    shf_delayRateLRDiffAll95 = NaN;
                    shf_delayRateLRDiffAll99 = NaN;
                    text(0.1,0.5,'Comparison not exist','FontSize',8);
                    Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_delayRateLRDiffAll(k) = 0;
                end
                Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_delayRateLRDiffAll95(k) = shf_delayRateLRDiffAll95;
                Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_delayRateLRDiffAll99(k) = shf_delayRateLRDiffAll99;
                ...
                if length(tempSort) > shuffleTimes/10
                    ...
                    hold off
                    Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_returnRateLRDiffAll(k) = 1;
                else
                    shf_returnRateLRDiffAll95 = NaN;
                    shf_returnRateLRDiffAll99 = NaN;
                    text(0.1,0.5,'Comparison not exist','FontSize',8);
                    Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_returnRateLRDiffAll(k) = 0;
                end
                Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_returnRateLRDiffAll95(k) = shf_returnRateLRDiffAll95;
                Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_returnRateLRDiffAll99(k) = shf_returnRateLRDiffAll99;
                ...
                if length(tempSort) > shuffleTimes/10
                    ...
                    hold off
                    Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_stemRateLRDiffAll(k) = 1;
                else
                    shf_stemRateLRDiffAll95 = NaN;
                    shf_stemRateLRDiffAll99 = NaN;
                    text(0.1,0.5,'Comparison not exist','FontSize',8);
                    Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_stemRateLRDiffAll(k) = 0;
                end
                Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_stemRateLRDiffAll95(k) = shf_stemRateLRDiffAll95;
                Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_stemRateLRDiffAll99(k) = shf_stemRateLRDiffAll99;
                ylabel('Stem (L vs R)')
                xticklabels({''});
                yticklabels({''});
                ...
                if length(tempSort) > shuffleTimes/10
                    ...
                    hold off
                    Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_ChoRateLRDiffAll(k) = 1;
                else
                    shf_ChoRateLRDiffAll95 = NaN;
                    shf_ChoRateLRDiffAll99 = NaN;
                    text(0.1,0.5,'Comparison not exist','FontSize',8);
                    Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_ChoRateLRDiffAll(k) = 0;
                end
                Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_ChoRateLRDiffAll95(k) = shf_ChoRateLRDiffAll95;
                Fig8TreadmillArmChoice_SameDelay.(behaveType).shf_ChoRateLRDiffAll99(k) = shf_ChoRateLRDiffAll99;
    ...
    if p.writeToFile
        save(fullfile(savedir2,'Fig8TreadmillArmChoice_SameDelay.mat'), 'Fig8TreadmillArmChoice_SameDelay');
    end
    clear Fig8TreadmillArmChoice_SameDelay map_1D
    close all
