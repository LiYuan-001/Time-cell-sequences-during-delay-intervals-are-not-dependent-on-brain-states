...
% Li Yuan, 27-Oct-2020
function Fig8Treadmill_Assembly_Oscillation(inFile,AnalyzeSes)
...
%     end
    ...
    Fig8_Assembly_SWR_Rate.session = sessInfo(i).mainDir;
    Fig8_Assembly_SWR_Rate.rat = sessInfo(i).animal;
    Fig8_Assembly_SWR_Rate.day = sessInfo(i).day;
...
%     [evtOnIdx.F,evtOffIdx.F,eegLabel.F] = eventFinder(EEG,eeg.F,p.Fs,1,0.05,0.05,0);
    ...
    event_swr_pow = cell(patNum,1);
    % calculate swr strength during the assembly event
    for k = 1:patNum
        event_Ts = event_Time{k};
        for mm = 1:length(event_Ts)
            ...
            event_swr_pow{k} = [event_swr_pow{k},pow_Temp];
        end
    end
    ...
    for j = 1:length(sessDirs)
        ...
        % label delay time with assembly bin with swr 
        bin_Time_SWR = [];
        bin_Time_Delay = [];
        ...
        for m = 1:length(delayTstart1)
            ...
            bin_Time_SWR_Temp = zeros(1,sum(bin_Time_Idx));
            
            for mm = 1:length(bin_Time_Temp)-1
                if sum(tSp>=bin_Time_Temp(mm) & tSp<bin_Time_Temp(mm+1)) > 0
                    bin_Time_SWR_Temp(mm) = 1;                  
                end                
            end            
            bin_Time_SWR = [bin_Time_SWR,bin_Time_SWR_Temp];
            bin_Time_Delay = [bin_Time_Delay,bin_Time_Temp];
        ...
        Fig8_Assembly_SWR_Rate.(sessDirs{j}).SWR.rateReturn = [rateReturn,sum(sum(time_All(:,1))),nansum(nansum(spike_All(:,1)))];
        Fig8_Assembly_SWR_Rate.(sessDirs{j}).SWR.rateBase = [rateBase,sum(sum(time_All(:,2))),nansum(nansum(spike_All(:,2)))];
        Fig8_Assembly_SWR_Rate.(sessDirs{j}).SWR.rateDelay = [rateDelay,sum(sum(time_All(:,3))),nansum(nansum(spike_All(:,3)))];
        Fig8_Assembly_SWR_Rate.(sessDirs{j}).SWR.rateStem = [rateStem,sum(sum(time_All(:,4))),nansum(nansum(spike_All(:,4)))];
        Fig8_Assembly_SWR_Rate.(sessDirs{j}).SWR.rateChoice = [rateChoice,sum(sum(time_All(:,5))),nansum(nansum(spike_All(:,5)))];
        Fig8_Assembly_SWR_Rate.(sessDirs{j}).SWR.rateReward = [rateReward,sum(sum(time_All(:,6))),nansum(nansum(spike_All(:,6)))];
        
        %% assembly rate and matching swr power
        pat_Delay_Event = zeros(patNum,length(bin_Time_SWR));
        pat_Delay_Strength = zeros(patNum,length(bin_Time_SWR));
        for k = 1:patNum
            ...
            for m = 1:length(delayTstart1)
                ...
                spikeInd = (tSp>=PathZone.posStartT.Return(m) & tSp<PathZone.posEndT.Return(m));
                spike_All(m,1) = sum(spikeInd);
                spik_swr_pow(m,1) = sum(event_swr_pow{k}(spikeInd));
                strength_All(m,1) = sum(strength(spikeInd));
                
                % base
                spikeInd = (tSp>=PathZone.posStartT.Base(m) & tSp<PathZone.posEndT.Base(m));
                spike_All(m,2) = sum(spikeInd);
                spik_swr_pow(m,2) = sum(event_swr_pow{k}(spikeInd));
                strength_All(m,2) = sum(strength(spikeInd));
                
                % delay
                spikeInd = (tSp>=Fig8DelayZonePos.delayPos1.startT(m) & tSp<delayTend1_2(m));
                spike_All(m,3) = sum(spikeInd);
                spik_swr_pow(m,3) = sum(event_swr_pow{k}(spikeInd));
                strength_All(m,3) = sum(strength(spikeInd));
                
                % stem
                spikeInd = (tSp>=(Fig8DelayZonePos.delayPos1.endT(m)+1/30) & tSp<PathZone.posEndT.Center(m));
                spike_All(m,4) = sum(spikeInd);
                spik_swr_pow(m,4) = sum(event_swr_pow{k}(spikeInd));
                strength_All(m,4) = sum(strength(spikeInd));
                
                % choice
                spikeInd = (tSp>=PathZone.posStartT.Choice(m) & tSp<PathZone.posEndT.Choice(m));
                spike_All(m,5) = sum(spikeInd);
                spik_swr_pow(m,5) = sum(event_swr_pow{k}(spikeInd));
                strength_All(m,5) = sum(strength(spikeInd));
                
                % reward
                spikeInd = (tSp>=PathZone.posStartT.Reward(m) & tSp<PathZone.posEndT.Reward(m));
                spike_All(m,6) = sum(spikeInd);
                spik_swr_pow(m,6) = sum(event_swr_pow{k}(spikeInd));
            ...
            Fig8_Assembly_SWR_Rate.(sessDirs{j}).Assembly.rateReturn(k,:) = [rateReturn,sum(sum(time_All(:,1))),nansum(nansum(spike_All(:,1))),sum(spik_swr_pow(:,1)),sum(strength_All(:,1))];
            Fig8_Assembly_SWR_Rate.(sessDirs{j}).Assembly.rateBase(k,:) = [rateBase,sum(sum(time_All(:,2))),nansum(nansum(spike_All(:,2))),sum(spik_swr_pow(:,2)),sum(strength_All(:,2))];
            Fig8_Assembly_SWR_Rate.(sessDirs{j}).Assembly.rateDelay(k,:) = [rateDelay,sum(sum(time_All(:,3))),nansum(nansum(spike_All(:,3))),sum(spik_swr_pow(:,3)),sum(strength_All(:,3))];
            Fig8_Assembly_SWR_Rate.(sessDirs{j}).Assembly.rateStem(k,:) = [rateStem,sum(sum(time_All(:,4))),nansum(nansum(spike_All(:,4))),sum(spik_swr_pow(:,4)),sum(strength_All(:,4))];
            Fig8_Assembly_SWR_Rate.(sessDirs{j}).Assembly.rateChoice(k,:) = [rateChoice,sum(sum(time_All(:,5))),nansum(nansum(spike_All(:,5))),sum(spik_swr_pow(:,5)),sum(strength_All(:,5))];
            Fig8_Assembly_SWR_Rate.(sessDirs{j}).Assembly.rateReward(k,:) = [rateReward,sum(sum(time_All(:,6))),nansum(nansum(spike_All(:,6))),sum(spik_swr_pow(:,6)),sum(strength_All(:,6))];
    ...
    if p.writeToFile
        save(fullfile(savedir2,'Fig8_Assembly_SWR_Rate-25ms.mat'), 'Fig8_Assembly_SWR_Rate');
    end
    
    fprintf('Finished spike shape analysis for session %d\n',i);
    clear Fig8_Assembly_SWR_Rate
end
end
