function Fig8Treadmill_Delay_PairwiseCorr_PCAICA(inFile,AnalyzeSes)
...
%             spkTrain(k,:) = zscore(fireTemp);
    ...
    for j = 1:length(sessDirs)
        ...
        corrTemp = zeros(trialNum,clusterNum,clusterNum);
        seqTotal_L = [];
        seqTotal_R = [];
        for m = 1:trialNum
            ...
            %                     seq1 = spkTrain(kk,delayBinIdx);
            %                     seq2 = spkTrain(mm,delayBinIdx);
            %                     corrTemp(m,kk,mm) = corr(seq1,seq2,'Rows','pairwise');
            %                     corrTemp(m,mm,kk) = corr(seq1,seq2,'Rows','pairwise');
            %                 end
            %             end
        ...
        figure(1)
        assemblyNum = size(AssmblWght_L,2);
        for kk = 1:assemblyNum
            subplot(assemblyNum,8,j+(kk-1)*8)
            stem(AssmblWght_L(:,kk))
            hold on
            stem(AssmblPtrnCellIDs_L{kk},AssmblWght_L(AssmblPtrnCellIDs_L{kk},kk),'r')
            if kk == 1
                title(strcat(sessDirs{j},'L assembly delay'))
            end
        end
        ...
        figure(3)
        assemblyNum = size(AssmblWght_R,2);
        for kk = 1:assemblyNum
            subplot(assemblyNum,8,j+(kk-1)*8)
            stem(AssmblWght_R(:,kk))
            hold on
            stem(AssmblPtrnCellIDs_R{kk},AssmblWght_R(AssmblPtrnCellIDs_R{kk},kk),'r')
            if kk == 1
                title(strcat(sessDirs{j},'R assembly delay'))
            end
        end
        figure(4)
        h1 = subplot(2,8,j);
