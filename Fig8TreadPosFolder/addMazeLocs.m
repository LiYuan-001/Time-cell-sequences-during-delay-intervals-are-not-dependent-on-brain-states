...
switch mazeType
    case {'fig8', 'fig8rat', 'fig8mouse','fig8Treadmill'}
...
%     noLocBnds = continuousRunsOfTrue(noLoc');
    ...
    % correction for those only few positions in another loc due to
    % definition of maze segments (here take 3 positions) 
    % Jan-07-2020, Li Yuan
    j = 1;
    i = 1;
    LOCtemp = locID(1);
    while i < length(locID)
        i = i+1;
        if locID(i) == LOCtemp
            j = j+1;
        else
            ...
            if j < 4
                if i-j-1>0
                locID(i-j:i-1) = locID(i-j-1);
                else
                    locID(i-j:i-1) = LOCtemp;
                end
            end
            j=1;
        end
